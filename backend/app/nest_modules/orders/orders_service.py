...
class OrdersService:
    ...
    def create_order(
        self, order_data: OrderCreate, user_id: int, business_id: int, db: Session
    ):
        ...
        for item_data in order_data.items:
            item_subtotal, item_dict = self._process_order_item(
                item_data, business_id, db
            )
        ...
        if order_data.payments and len(order_data.payments) > 0:
            total_pagado = self._add_payments(
                order, order_data.payments, business_id, db
            )
        ...
        # Generar asiento contable automático si la orden está pagada
        if order.payment_status == "paid" and self.accounting_integration:
            try:
                period_id = None  # Se determinará automáticamente en el servicio
                self.accounting_integration.create_sale_entry(
                    order, business_id, user_id, db, period_id
                )
            except Exception as e:
                # No fallar la creación de orden si falla el asiento contable
    ...
    def _process_menu_item(self, item_data, business_id: int, db: Session):
        ...
        for ingredient in menu_item.ingredients:
            ingredient_qty = menu_item_repo.get_ingredient_quantity(
                menu_item.id, ingredient.id
            )
    ...
    def _enrich_with_customer_info(self, order):
        ...
        if order.customer:
            ...
            order.customer_name = (
                f"{order.customer.nombre} {order.customer.apellido or ''}".strip()
            )
    ...
    def update_order(
        self, order_id: int, order_update: OrderUpdate, business_id: int, db: Session
    ):
        """Actualizar orden"""
        order_repo = OrderRepository(db)
        table_repo = TableRepository(db)
    ...
    def add_payments_to_order(
        ...
        # Generar asiento contable automático si la orden se marca como pagada
        if order.payment_status == "paid" and self.accounting_integration:
            try:
                period_id = None
