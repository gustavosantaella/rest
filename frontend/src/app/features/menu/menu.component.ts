...
export class MenuComponent implements OnInit {
  ...
  initForms(): void {
    ...
    // Suscribirse a cambios en el precio para actualizar margen de ganancia
  ...
  addIngredient(): void {
    ...
    // Suscribirse a cambios para actualizar costo en tiempo real
    ingredient.get("product_id")?.valueChanges.subscribe(() => {
      this.cdr.detectChanges();
    });
    ingredient.get("quantity")?.valueChanges.subscribe(() => {
      this.cdr.detectChanges();
    });
  ...
  getProductPurchasePrice(productId: number): number {
    const product = this.products.find((p) => p.id === productId);
  ...
  calculateProductionCost(): number {
    ...
    this.ingredientsArray.controls.forEach((control) => {
      ...
      if (productId && quantity > 0) {
        const purchasePrice = this.getProductPurchasePrice(Number(productId));
  ...
  get profitAmount(): number {
    const price = this.itemForm.get("price")?.value || 0;
    return price - this.productionCost;
  }
